...
# CORS pentru frontend Live Server / browser
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:5500",
...
def creeaza_token(date: dict) -> str:
    ...
    date_copie["exp"] = datetime.now(timezone.utc) + timedelta(
...
def inregistrare(
    ...
    return {
        "mesaj": f"Utilizatorul {utilizator.email} a fost înregistrat cu succes."
    }
...
def autentificare(
    ...
    utilizator = db.execute(
        "SELECT * FROM utilizatori WHERE email = ?",
        (formular.username.lower(),)
    ).fetchone()

    if not utilizator or not verifica_parola(
        formular.password,
        utilizator["parola_hash"]
    ):
        raise HTTPException(
            status_code=401,
...
# ---------------------------------------------------------------------------
# Endpoint bonus: utilizator curent
# ---------------------------------------------------------------------------
...
def obtine_utilizator_curent(
    ...
    return {
        "id": utilizator_curent["id"],
        "email": utilizator_curent["email"]
    }
...
def obtine_sarcini(
    ...
    return [dict(sarcina) for sarcina in sarcini]
...
def creeaza_sarcina(
    ...
    cursor = db.execute(
        """
        INSERT INTO sarcini (titlu, descriere, finalizata, data_crearii, utilizator_id)
        VALUES (?, ?, ?, ?, ?)
        """,
        (
            sarcina.titlu,
...
def marcheaza_sarcina_finalizata(
    sarcina_id: int,
...
def finaliza_sarcina(
    sarcina_id: int,
    db: sqlite3.Connection = Depends(get_db),
    utilizator_curent=Depends(get_utilizator_curent),
):
    return marcheaza_sarcina_finalizata(sarcina_id, db, utilizator_curent)
...
def sterge_sarcina(
    ...
    return {
        "mesaj": f"Sarcina cu ID-ul {sarcina_id} a fost ștearsă."
    }



# Trebuie sa ramana ultimul apel: serveste frontend-ul si rutele SPA nerezolvate.
